...
├── fetch_logos.py        # 백그라운드 — 종목 로고 다운로드
├── fetch_master.py       # 백그라운드 — 미국 종목 마스터 수집
└── tossinvest/
    ...
    ├── jobs.py           # 결과 출력 후 도는 준비 작업 (선점 표식·프로세스 분리)
    ├── logos.py          # 종목 로고 캐시 (아이콘 경로)
...
## 목록을 그리는 경로에서 네트워크를 타지 않는다

Script Filter 는 키 입력마다, 그리고 자동 갱신 주기(기본 2초)마다 실행된다. 여기에
느린 작업을 하나 넣으면 그 비용을 사용자가 매번 낸다.

오래 걸리는 준비 작업은 `tossinvest/jobs.py` 에 쌓고, `alfred.output()` 이 결과를
**stdout 으로 내보낸 뒤** `jobs.run_pending()` 으로 분리된 프로세스에 넘긴다.
결과는 다음 자동 갱신에 반영된다. 지금 이 방식으로 도는 것은 두 가지다.

- `fetch_logos.py` — 종목 로고 다운로드
- `fetch_master.py` — 미국 종목 마스터 수집

새 준비 작업도 같은 자리에 붙인다. 지켜야 할 것이 두 가지 있다.

- **선점 표식**(`jobs.claim`) — 자동 갱신이 2초라 표식이 없으면 아직 받는 중인
  대상을 다음 실행이 또 요청한다. 표식은 성공·실패와 무관하게 반드시 반납한다.
- **세션 분리**(`start_new_session`) — 사용자가 키를 더 누르면 Alfred 가 지금
  Script Filter 를 죽인다. 자식이 같은 세션에 있으면 함께 죽어서 준비 작업이
  영영 끝나지 않는다.

받아지기 전 상태에서도 화면이 쓸모 있어야 한다. 로고가 없으면 기존 아이콘으로,
미국 마스터가 없으면 국내 종목만으로 동작한다.

## `/stocks/all` 은 초당 1회다

문서의 rate limit 표에 이 그룹이 없다. 응답 헤더의 `X-RateLimit-Limit` 이 1 이고,
시장을 연달아 받으면 두 번째부터 429 가 난다. 캐시가 살아 있는 동안에는 멀쩡하다가
만료 직후에만 터지고 그때 **검색 결과가 통째로 빈다.**

`api.fetch_master()` 가 호출 간격을 지키고 429 를 한 번 재시도한다. `/stocks/all`
을 이 함수 밖에서 직접 부르지 않는다.

국내 시장은 검색의 알맹이라 없으면 그 자리에서 받고(하루 한 번, 약 2초), 미국
시장은 `fetch_master.py` 에 맡긴다. 네 시장을 다 동기로 받으면 첫 실행이 몇 초씩
멈춘다.
